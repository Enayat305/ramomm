...
class Install extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            if ($this->input->post('step') == 3) {
                ...
                $this->form_validation->set_rules('purchase_username', 'Envato Username', 'trim');
                $this->form_validation->set_rules('purchase_code', 'Purchase Code', 'trim|callback_purchase_validation');
                ...
                if ($this->form_validation->run() == true) {
					...
					if ( !isset($purchaseCode->status) && $purchaseCode->status ) {
						if (!empty($purchaseCode->sql)) {
							...
							if (mysqli_multi_query($this->db->conn_id, 
							)) {
    ...
    public function purchase_validation()
    {
        if($purchase_code = "") {
